...
type MaterialSymbol = (typeof Icons)[number];

const GlyphMap = Icons.reduce(
  (acc, icon) => {
    acc[icon] = icon.replaceAll('-', '_');
    return acc;
  },
  {} as Record<MaterialSymbol, string>,
);

const isIcon = (icon: string): icon is MaterialSymbol => Icons.includes(icon as MaterialSymbol);
...
function Icon(props: IconProps) {
  const { name, size = 24, color = 'black', variant = 'outlined', style } = props;
  styles.useVariants({ variant });

  return (
    <View style={styles.root(size)}>
      <RNText style={[styles.text(size, color), style]}>{isIcon(name) ? GlyphMap[name] : name}</RNText>
    </View>
  );
}
...
const styles = StyleSheet.create((theme) => ({
  ...
  text: (size: number, color: string | undefined) => ({
    color: color || theme.scheme.onBackground,
    fontSize: size,
    lineHeight: size * 1.1,
    width: size,
    height: size,
    ...
    variants: {
      variant: {
        ...
        rounded: {
          fontFamily: 'MaterialSymbolsRounded-Regular',
        },
        sharp: {
          fontFamily: 'MaterialSymbolsSharp-Regular',
        },
      },
    },
  }),
...
export type { IconProps, IconVariant, MaterialSymbol };
export { GlyphMap, Icon, Icons, isIcon };
